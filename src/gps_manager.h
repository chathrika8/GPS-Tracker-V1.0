...
struct DeviceState {
  ...
  // SMS — surfaced on diagnostic display
  uint8_t sms_count;        // number of SMS in buffer
  uint8_t sms_unread_count; // unread count for badge
  char sms_sender[10][20];  // sender numbers
  char sms_preview[10][24]; // first 23 chars of body + null
  char sms_body[10][161];   // full body text
  char sms_time[10][20];    // timestamps
  bool sms_unread[10];      // read/unread flags
  uint8_t msg_sub_screen;   // 0 = SMS list, 1 = SMS reader
  uint8_t msg_selected_sms; // index of selected SMS for reader view
};
...
public:
  ...
  void saveLastPositionToNVS(double lat, double lon, float altM,
                             uint32_t epoch);
  bool loadLastPositionFromNVS(double *lat, double *lon, float *altM,
                               uint32_t *epoch);
...
private:
  ...
  uint32_t computeUnixEpoch(int year, int month, int day, int hour, int minute,
                            int second);
};
